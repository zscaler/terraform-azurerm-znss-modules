...
curl -sL https://github.com/tfsec/tfsec/releases/download/v1.26.3/tfsec-linux-amd64 > tfsec    & \
curl -sL https://github.com/terraform-linters/tflint/releases/download/v0.39.1/tflint_linux_amd64.zip > tflint.zip    & \
...
curl -sL https://releases.hashicorp.com/terraform/1.2.6/terraform_1.2.6_linux_amd64.zip > terraform-1.2.6.zip    & \
wait
...
unzip terraform-1.2.6.zip
rm terraform-1.2.6.zip
mv terraform /usr/local/bin/terraform
